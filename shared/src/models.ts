...
// 具体模型 id（透传给 claude CLI 的 --model）。别名 preset（sonnet/opus/fable）
// 由 CLI 解析为"当前默认版本"，而这里的 id 固定锁定某一代模型，用于在
// opus 4.8/4.7/4.6 等具体版本之间切换。列表允许过期：任何字符串最终都
// 原样透传，UI 另提供自定义 id 输入兜底。
