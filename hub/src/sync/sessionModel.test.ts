...
describe('session model', () => {
    ...
    it('passes stored Copilot agent mode when respawning a resumed Copilot session', async () => {
        ...
        try {
            ...
            ;(engine as any).rpcGateway.spawnSession = async (
                ...
                _serviceTier?: string,
                _claudeLaunch?: unknown,
                _ccSwitchProviderId?: string,
    ...
    describe('runner stop confirmation', () => {
        it('rejects orchestration stop while the process is still alive', async () => {
            const engine = new SyncEngine(
                new Store(':memory:'),
                {} as never,
                new RpcRegistry(),
            ...
            Object.defineProperty(engine, 'rpcGateway', {
                value: { stopRunnerSession: async () => 'still_alive' }
            })

            await expect(engine.stopSessionOnMachine('machine-1', 'session-1'))
                .rejects.toThrow('Session process is still alive')
        })

        it('does not mark or resume a live session after stop refusal', async () => {
            const engine = new SyncEngine(
                new Store(':memory:'),
                {} as never,
                new RpcRegistry(),
            ...
            const resumeSpy = spyOn(engine, 'resumeSession').mockImplementation(async () => {
                throw new Error('restart must not continue')
            })

            await expect(engine.restartSession(session.id, 'default')).resolves.toEqual({
                type: 'error',
