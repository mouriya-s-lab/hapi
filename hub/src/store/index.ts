...
export class Store {
    ...
    private migrateFromV9ToV10(legacy: boolean = false): void {
        // Multi-user: add owner_account_id to machines/sessions and create the
        // accounts / api_tokens / resource_grants tables. New columns are
        // nullable so every existing row keeps working; ownership backfill to a
        // bootstrap admin happens at startup (see bootstrapMultiUser), not here,
        // because the migration layer has no knowledge of the legacy token.
        ...
        if (machineColumns.size > 0 && !machineColumns.has('owner_account_id')) {
            ...
            // Regular upgrade path expects the machines table to already exist.
            throw new Error('SQLite schema missing machines table for v9 to v10 migration.')
        }
        ...
        // New tables — idempotent via IF NOT EXISTS, mirrors createSchema.
        this.db.exec(`
            CREATE TABLE IF NOT EXISTS accounts (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                username TEXT NOT NULL UNIQUE,
