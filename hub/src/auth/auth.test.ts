...
describe('machine-access session inheritance', () => {
    function setup() {
        const store = new Store(':memory:')
        ...
        const outsider = store.accounts.create({ username: 'outsider', passwordHash: null, role: 'user', defaultNamespace: 'default' })
        store.machines.getOrCreateMachine('vircs', { host: 'vircs' }, null, 'default', admin.id)
        const session = store.sessions.getOrCreateSession(
            'on-vircs',
            { path: '/tmp/p', host: 'vircs', machineId: 'vircs' },
            null,
            'default',
            undefined,
            undefined,
        ...
        store.grants.upsert({ resourceType: 'machine', resourceId: 'vircs', granteeAccountId: peter.id, role: 'operator' })
        return { store, admin, peter, outsider, session }
    }

    it('machine grantee gets viewer (not operator) on sessions running there', () => {
        const { store, peter, outsider, session } = setup()
        try {
            const level = resolveAccessLevel({
                store, accountId: peter.id, role: 'user',
            ...
            expect(canOperate(level)).toBe(false)

            const none = resolveAccessLevel({
                store, accountId: outsider.id, role: 'user',
                resourceType: 'session', resourceId: session.id,
                ownerAccountId: session.ownerAccountId
            })
    ...
    it('getSessionsForAccount lists sessions on granted and owned machines', () => {
        const { store, admin, peter, outsider, session } = setup()
        try {
            const forPeter = store.sessions.getSessionsForAccount('default', peter.id)
            expect(forPeter.map((s) => s.id)).toContain(session.id)
            const forOutsider = store.sessions.getSessionsForAccount('default', outsider.id)
            expect(forOutsider.map((s) => s.id)).not.toContain(session.id)
            // Machine owner sees sessions others spawned on their machine.
            const peterSession = store.sessions.getOrCreateSession(
                'peter-on-vircs',
                { path: '/tmp/q', host: 'vircs', machineId: 'vircs' },
                null, 'default', undefined, undefined, undefined, peter.id
            )
            const forAdminOwner = store.sessions.getSessionsForAccount('default', admin.id)
            expect(forAdminOwner.map((s) => s.id)).toContain(peterSession.id)
        } finally {
            store.close()
        }
    })

    it('listReadableAccountIds includes machine owner and grantees for SSE/push fan-out', async () => {
        const { store, admin, peter, session } = setup()
        try {
            const { listReadableAccountIds } = await import('./access')
            const ids = listReadableAccountIds(store, 'session', session.id)
            expect(ids.has(admin.id)).toBe(true)
            expect(ids.has(peter.id)).toBe(true)
        } finally {
            store.close()
        }
    })
