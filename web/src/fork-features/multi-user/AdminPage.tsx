...
type Account = { id: number; username: string; role: 'admin' | 'user'; defaultNamespace: string; disabledAt: number | null; memory: string | null }
...
export default function AdminPage() {
    ...
    const [memoryTarget, setMemoryTarget] = useState<Account | null>(null)
    ...
    const refresh = useCallback(async () => {
        ...
        setIsLoading(true)
        try {
            const [tokenResult, machineResult, memoryResult] = await Promise.all([
                request<{ tokens: Token[] }>('/api/tokens'), request<{ machines: Machine[] }>('/api/machines'), request<{ memory: string | null }>('/api/memory')
            ])
            setTokens(tokenResult.tokens)
            setMachines(machineResult.machines)
            setMyMemory(memoryResult.memory ?? '')
    ...
    const saveMyMemory = async () => {
        await runAction('my-memory', async () => {
            const result = await request<{ memory: string | null }>('/api/memory', { method: 'PATCH', body: JSON.stringify({ memory: myMemory.trim() || null }) })
            setMyMemory(result.memory ?? '')
        })
    }

    const saveAccountMemory = async () => {
        if (!memoryTarget) return
        await runAction(`memory-${memoryTarget.id}`, async () => {
            await request(`/api/accounts/${memoryTarget.id}`, { method: 'PATCH', body: JSON.stringify({ memory: memoryDraft.trim() || null }) })
            setMemoryTarget(null); await refresh()
        })
    }
    ...
    return <div className="h-full min-h-0 overflow-y-auto bg-[var(--app-bg)] text-[var(--app-fg)]">
        ...
        <main className="mx-auto max-w-5xl space-y-4 p-4">
            ...
            {user.role === 'admin' && <Card>
                ...
                <CardContent className="space-y-2">{accounts.map(account => <div key={account.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[var(--app-border)] p-3"><div><div className="flex items-center gap-2"><b>{account.username}</b><Badge variant={account.role === 'admin' ? 'success' : 'default'}>{account.role}</Badge>{account.disabledAt && <Badge variant="destructive">disabled</Badge>}</div><div className="text-xs text-[var(--app-hint)]">namespace: {account.defaultNamespace}</div></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={pendingAction !== null || account.id === user.id} onClick={() => updateAccount(account.id, { role: account.role === 'admin' ? 'user' : 'admin' })}>{account.role === 'admin' ? '降为用户' : '升为管理员'}</Button><Button size="sm" variant="outline" disabled={pendingAction !== null} onClick={async () => { const next = window.prompt(`为 ${account.username} 设置新密码（至少 8 位）`); if (next && next.length >= 8) await updateAccount(account.id, { password: next }) }}>设密码</Button>{account.id !== user.id ? <Button size="sm" variant="outline" disabled={pendingAction !== null} onClick={() => { setMemoryTarget(account); setMemoryDraft(account.memory ?? '') }}>记忆</Button> : null}<Button size="sm" variant="outline" disabled={pendingAction !== null || account.id === user.id} onClick={() => updateAccount(account.id, { disabled: !account.disabledAt })}>{account.disabledAt ? '启用' : '禁用'}</Button><Button size="sm" variant="destructive" disabled={pendingAction !== null || account.id === user.id} onClick={async () => { if (window.confirm(`确定删除 ${account.username}？`)) await runAction(`delete-${account.id}`, async () => { await request(`/api/accounts/${account.id}`, { method: 'DELETE' }); await refresh() }) }}>删除</Button></div></div>)}</CardContent>
            ...
            <Card><CardHeader><CardTitle>我的记忆</CardTitle><CardDescription>随你发送的消息提供给 agent；聊天记录仍保留原文。</CardDescription></CardHeader><CardContent className="space-y-2"><textarea className={`${inputClass} min-h-24 resize-y`} maxLength={4000} value={myMemory} placeholder="例如：我的电脑是 ALICE-PC" onChange={event => setMyMemory(event.target.value)} /><div className="flex justify-end"><Button size="sm" disabled={pendingAction !== null} onClick={saveMyMemory}>{pendingAction === 'my-memory' ? '保存中…' : '保存'}</Button></div></CardContent></Card>
            ...
            <Dialog open={memoryTarget !== null} onOpenChange={open => { if (!open && pendingAction === null) setMemoryTarget(null) }}><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>用户记忆：{memoryTarget?.username}</DialogTitle></DialogHeader><div className="space-y-3"><textarea aria-label="用户记忆" className={`${inputClass} min-h-32 resize-y`} maxLength={4000} value={memoryDraft} placeholder="随该用户发送的消息提供给 agent" onChange={event => setMemoryDraft(event.target.value)} /><div className="flex justify-end gap-2"><Button variant="outline" disabled={pendingAction !== null} onClick={() => setMemoryTarget(null)}>取消</Button><Button disabled={pendingAction !== null} onClick={saveAccountMemory}>{pendingAction?.startsWith('memory-') ? '保存中…' : '保存'}</Button></div></div></DialogContent></Dialog>
        </main>
    </div>
}
