export default {
  ...
  'omp.login.title': 'OMP 提供商登录',
  'omp.login.description': '可选 OAuth 提供商已就绪。登录凭据只保存在 OMP，不会写入聊天。',
  'omp.login.provider': '选择提供商',
  'omp.login.action': '登录 {provider}',
  'omp.login.submit': '登录',
  'omp.login.pending': '请在聊天中的登录卡片完成提供商流程。',
  'omp.login.authenticated': '已登录：{providers}',
  'omp.login.error': 'OMP 登录失败：{message}',
