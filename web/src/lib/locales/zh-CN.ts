export default {
  ...
  'usage.subtitle': '按模型汇总的输入/输出/缓存 token 用量',
  'usage.nav': 'Token 用量统计',
  'usage.back': '返回',
  'usage.range.all': '全部',
  ...
  'usage.host.label': '按机器筛选',
  'usage.host.all': '全部机器',
  'usage.refreshing': '刷新中…',
  'usage.loading': '加载中…',
  'usage.error.title': '加载失败',
  'usage.error.fallback': '无法获取用量数据',
  'usage.empty.title': '暂无数据',
  'usage.empty.hintAll': '还没有任何会话产生带 usage 信息的消息。',
  'usage.empty.hintFiltered': '当前筛选条件下没有用量记录，试试放宽时间范围或切换机器。',
  'usage.overview': '总览',
  'usage.overviewSummary': '共 {requests} 次请求 · {tokens} tokens',
  'usage.detail': '明细',
  'usage.legend.input': '输入 (input)',
  'usage.legend.output': '输出 (output)',
  'usage.legend.cacheCreation': '缓存写入 (cache creation)',
  'usage.legend.cacheRead': '缓存读取 (cache read)',
