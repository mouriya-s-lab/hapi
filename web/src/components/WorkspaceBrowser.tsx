...
export function WorkspaceBrowser(props: {
    ...
    const handleCreateDirectory = useCallback(async () => {
        if (!machineId || !currentPath || !newDirectoryName.trim()) return
        setIsLoading(true)
        setError(null)
        try {
            const result = await api.createMachineDirectory(machineId, currentPath, newDirectoryName.trim())
    ...
    }, [api, currentPath, loadDirectory, machineId, newDirectoryName])
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            {currentPath && isCreatingDirectory && (
                <form
                    className="flex items-center gap-2 px-3 py-2 border-b border-[var(--app-divider)]"
                    onSubmit={(event) => {
                ...
                >
                    <input
                        ...
                        placeholder={t('browse.newDirectoryPlaceholder')}
                        disabled={isLoading}
                        className="min-w-0 flex-1 rounded-md border border-[var(--app-border)] bg-[var(--app-bg)] px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--app-link)]"
                    />
                    <button
                        type="button"
                        onClick={() => {
                        ...
                        className="px-2 py-1.5 text-sm text-[var(--app-hint)]"
                    >
                        {t('button.cancel')}
                    </button>
                    <button
                        ...
                        className="px-3 py-1.5 text-sm rounded-md bg-[var(--app-button)] text-[var(--app-button-text)] disabled:opacity-50"
                    >
                        {t('browse.createDirectory')}
                    </button>
                </form>
            )}
                ...
                <div className="px-3 py-2 border-t border-[var(--app-divider)]">
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            disabled={isLoading || isCreatingDirectory}
                            className="flex items-center gap-1 px-2 py-1.5 text-xs rounded-lg border border-[var(--app-border)] text-[var(--app-fg)] hover:bg-[var(--app-subtle-bg)] disabled:opacity-50"
                            title={t('browse.newDirectory')}
                        >
                            <FolderPlusIcon className="h-3.5 w-3.5" />
                            <span className="hidden sm:inline">{t('browse.newDirectory')}</span>
                        </button>
                        <button
                            type="button"
                            onClick={() => void copyPath(currentPath)}
                            className="flex items-center gap-1 px-2 py-1.5 text-xs rounded-lg border border-[var(--app-border)] text-[var(--app-fg)] hover:bg-[var(--app-subtle-bg)]"
                            title={t('browse.copyPath')}
                        >
                            <CopyIcon className="h-3.5 w-3.5" />
                            <span className="hidden sm:inline">{pathCopied ? t('button.copied') : t('browse.copyPath')}</span>
                        </button>
                        <button
                            type="button"
