...
| `hub/src/socket/handlers/cli/index.ts`, `hub/src/socket/handlers/cli/sessionHandlers.ts` | Accept a metadata sanitiser so injected gateway memory cannot come back as a CLI-reported session title. |
...
| `hub/src/socket/handlers/cli/index.ts`, `hub/src/socket/handlers/cli/sessionHandlers.ts` | The CLI metadata handler exposes no write-time metadata filter registry | The gateway prepends account memory to the CLI-bound prompt, and upstream's `applySessionTitleFallback` turns that prompt into the session title — so the injected block must be stripped at the one point where CLI-reported metadata is persisted | Web message → gateway memory decoration → CLI title fallback → `update-metadata` → sanitiser → session store | Send a web message from an account that has memory and confirm the sidebar title is the prompt, not `<hapi_user_context …>` |
