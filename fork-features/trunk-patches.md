...
| `cli/src/agent/messageConverter.ts`, `cli/src/agent/types.ts`, `cli/src/commands/resume.ts`, `cli/src/runner/run.ts` and adjacent tests | No external agent-message metadata or flavor launch-argument registry | Native provider/model/usage/cost must decorate the existing visible-message ADT, while model/thinking selections must cross the closed runner and resume argument builders | OMP assistant message → AgentMessage conversion → persisted visible footer; Web resume → runner args → OMP launch | Complete a tool-call turn and compare provider/model/tokens/cost with the native transcript; reconnect with configured `auto` |
...
| `web/src/api/client.ts`, `web/src/chat/normalizeAgent.ts`, `web/src/chat/types.ts`, `web/src/components/AssistantChat/HappyComposer.tsx`, `StatusBar.tsx`, `messages/MessageMetadata.tsx`, `modelOptions.ts`, `web/src/components/SessionChat.tsx`, `web/src/lib/assistant-runtime.ts`, `query-keys.ts`, locale/type files, and adjacent tests | No flavor-specific composer, status, normalized-message metadata, or query registration surface | Dynamic provider-qualified options, configured/resolved thinking, provider usage/cost, and the Claude-only boundary occupy closed existing chat and composer lifecycle slots | OMP catalog/state/message events → authenticated query/normalizer → composer/status/message footer | In a real browser switch model and low/high/auto, inspect context/cache/model/cost, and confirm the Claude-only warning |
