...
| `cli/src/codex/codexRemoteLauncher.ts`, `codexEventConverter.ts` and tests; `fork-features/codex-compact-summary/turnOwnership.ts`; `web/src/chat/normalize.test.ts`, `presentation.ts`, `SystemMessage.tsx` | No Codex compact-summary event, turn-ownership, or renderer registry | Compact events must traverse the existing launcher, which alone owns app-server turn IDs, before entering the normalized chat ADT and system-message renderer | Codex compact event → HAPI turn-ownership gate → normalized expandable system message | Trigger real HAPI-owned and external Codex compaction/retry; confirm only the owned summary appears and expands; re-run linked upstream retry issues |
...
| `PwaUpdateBanner.tsx`, `hooks/usePwaUpdate.ts`, `hooks/usePwaUpdate.test.ts`, `routes/settings/about.tsx`, locale files, and tests | No PWA update-banner content slot, About-page action registry, or reload-policy injection seam | Fork changelog content and manual reload action use the existing update lifecycle; the hook privately owns `needRefresh` and `updateSW`, so immediate navigation without a pending worker versus pending-worker activation cannot be extracted | service-worker update or About page → shared reload action → immediate navigation or worker activation → version activation | Build two versions, trigger update, reload from the banner and About page, and confirm the new build activates |
...
configuration, typed input queue, prompt, and terminal display logic live in
`cli/src/omp/`; the command implementation lives in `cli/src/commands/omp.ts`.
The active remote
path is OMP 17.0.4 RPC, not ACP, and both remote and local child processes are
fixed to native yolo execution because RPC exposes no permission/plan channel.
Its durable native-session snapshot is
...
| `cli/src/agent/loopBase.ts`, `cli/src/agent/sessionBase.ts` | No queue-provider interface on the shared local/remote session loop | OMP must preserve typed atomic commands and image metadata instead of collapsing input into `MessageQueue2` string batches; the shared loop now accepts the minimum queue/session products without importing OMP | Web message → typed OMP queue → local/remote launcher handoff → native RPC command | Queue multiple typed inputs, switch local/remote, and verify order, cancellation, and consumed IDs |
| `cli/src/api/types.ts`, `shared/src/apiTypes.ts`, `shared/src/schemas.ts`, `shared/src/types.ts`, `hub/src/web/routes/messages.ts`, `hub/src/sync/messageService.ts`, `hub/src/sync/syncEngine.ts`, `web/src/api/client.ts`, `web/src/types/api.ts` and adjacent tests | No message-metadata or send-request variant registry | The four OMP input commands must remain a validated discriminant across Web, REST, persistence, Hub-to-CLI delivery, and CLI parsing; an out-of-band channel would break ordering and cancellation | Web send request → Hub message metadata → CLI `OmpInputQueue` → OMP prompt/steer/follow-up/abort-and-prompt RPC | Send every input mode in a live OMP stream, cancel a held follow-up, and compare Hub order with the native transcript |
...
| `shared/src/flavors.ts`, `shared/src/flavors.test.ts`, `shared/src/modes.ts` | No flavor/mode extension registry across package boundaries | OMP must remain a validated shared discriminant and expose only the `default`/`yolo` labels while the native process stays fixed-yolo; a sibling module cannot extend the closed union at runtime | Web spawn payload → shared validation → runner args → fixed-yolo OMP launch | Confirm Web exposes only default/yolo, plan is rejected, and both labels execute direct Write/Bash without approval |
...
| `shared/package.json`, `shared/src/apiTypes.ts`, `shared/src/flavors.ts`, `shared/src/index.ts`, `shared/src/rpcMethods.ts`, `shared/src/schemas.ts`, `shared/src/types.ts` and adjacent tests | No flavor capability, RPC method, API product, or metadata-extension registry | Provider-qualified OMP models and the three-field thinking state must be the same typed contract in CLI, Hub, persistence, and Web; a fork-only parallel schema would split those boundaries | OMP native model/thinking response → shared schema/RPC product → Hub persistence → Web selector/status | Switch model and low/high/auto in a live OMP session, reconnect, and compare persisted `configured`/`resolved` state with native RPC state |
| `cli/src/agent/messageConverter.ts`, `cli/src/agent/types.ts`, `cli/src/claude/types.ts`, `cli/src/commands/resume.ts`, `cli/src/runner/run.ts` and adjacent tests | No external agent-message metadata, canonical output usage-schema, or flavor launch-argument registry | Native provider/model/usage/reasoning/cost must decorate the existing visible-message ADT and canonical `RawJSONLines` record, while model/thinking selections must cross the closed runner and resume argument builders | OMP assistant message → canonical output / AgentMessage conversion → persisted visible footer; Web resume → runner args → OMP launch | Complete a reasoning and tool-call turn, compare provider/model/tokens/reasoning/cost and display/provider IDs with the native transcript, then reconnect with configured `auto` |
| `hub/src/sync/rpcGateway.ts`, `hub/src/sync/syncEngine.ts`, `hub/src/web/routes/sessions.ts` and adjacent tests | No session RPC adapter or flavor-specific route registration API | OMP catalog, thinking options, and native model cycling must use the existing authenticated session boundary and feed confirmed config back into the cached per-turn session state | Web model/thinking control or shortcut → Hub route → session RPC → OMP mutation/get_state → session config acknowledgement | Select and cycle models in a live session, send a later turn, and verify the native model does not roll back |
| `web/src/api/client.ts`, `web/src/chat/normalizeAgent.ts`, `presentation.ts`, `reducerTimeline.ts`, `types.ts`, `web/src/components/AssistantChat/HappyComposer.tsx`, `StatusBar.tsx`, `messages/MessageMetadata.tsx`, `modelOptions.ts`, `web/src/components/SessionChat.tsx`, `web/src/lib/assistant-runtime.ts`, `query-keys.ts`, locale/type files, and adjacent tests | No flavor-specific composer, event presentation, agent-run reducer, normalized-message metadata, or query registration surface | Dynamic provider-qualified options, configured/resolved thinking, provider usage/reasoning/cost, unknown-event warnings, subagent retry ownership, and the Claude-only boundary occupy closed existing chat and composer lifecycle slots | OMP catalog/state/canonical message/structured event → authenticated query/normalizer → timeline reducer → composer/status/message footer | In a real browser switch model and low/high/auto, inspect context/cache/model/reasoning/cost, verify one owned subagent retry card, and display an injected unknown-frame warning |

Each upstream sync must re-check for native flavor/command, launch-policy,
queue-provider, message-metadata, metadata-variant, resume-projection,
...
| `cli/src/runner/run.ts`, `cli/src/agent/sessionFactory.ts`, `cli/src/api/api.ts`, `cli/src/api/apiMachine.ts`, `shared/src/schemas.ts` | No runner capability-probe or machine-RPC registration API | The fork-owned machine integration must probe the real OMP binary before advertising capability, register only on supported installations, synchronize that capability through the closed machine metadata product, and close an active login process during runner shutdown | runner start → `omp --version` probe → machine capability metadata/conditional OMP handlers → provider login/model discovery → runner shutdown | Start runners with supported, missing, and outdated OMP; verify only supported runners appear in OMP settings/New Session, then stop without a leaked OMP child process |
...
| `shared/src/apiTypes.ts`, `rpcMethods.ts`, `hub/src/sync/machineCache.ts`, `rpcGateway.ts`, `syncEngine.ts`, `hub/src/web/routes/machines.ts`, `hub/src/web/routes/machines.test.ts` | No current-runner capability registration lifecycle, machine-RPC product, or route registration API | Provider state, login flow, required input, and workspace-qualified model catalogs must cross one authenticated runner boundary without creating a HAPI session; repeated registration must replace the persisted OMP capability so an older runner cannot inherit a stale supported bit | runner registration → current OMP capability reconciliation → Web settings/New Session request → Hub capability guard → runner RPC → sessionless OMP process | Restart one machine ID from a supported runner into an older capability-absent runner, confirm OMP becomes unavailable, then list providers, complete callback/manual-input login, load the selected workspace's models, and reject invalid machine/input/cwd requests |
| `web/src/api/client.ts`, `web/src/lib/query-keys.ts`, `web/src/components/NewSession/index.tsx`, locale files, and tests | No machine-model selector or settings-contribution registry | Authenticated provider models must populate the existing New Session selector, while machine capability filtering prevents unsupported runners from receiving OMP RPCs and authentication changes invalidate any cached runner catalog | runner capability metadata → filtered settings/New Session runner list → provider authentication → model-cache invalidation → provider-qualified model selection → runner spawn | Mix supported and unsupported runners, verify only supported runners are selectable and missing handlers never surface as 500, then authenticate and create a session with the exact selected provider model |
...
| `hub/src/store/index.ts` | Reconcile the private V10→V11 table migration when opening V11 and commit the migration ladder only after required-table validation; the Store exposes no migration-step or pre-commit validation registration seam. |
...
| `hub/src/push/pushService.ts`, `hub/src/push/pushNotificationChannel.ts` | Accept a fork-provided endpoint audience so migrated account ownership continues to constrain Web Push delivery without moving multi-user policy into the upstream notification implementation. |
...
| `hub/src/store/index.ts` | No migration-step registration or pre-final-version validation seam | PR #102 cleanup can expose a historically stamped V11 core shape, while only private `Store.initSchema` can reconcile the idempotent FCM step and atomically commit the final version | fork compatibility cleanup → core migration ladder → required-table validation → `user_version` commit | Start a V11 database without `fcm_devices` and verify V15 plus the table; remove another required table and verify failure leaves V11 unchanged |
| `hub/src/push/pushService.ts`, `hub/src/push/pushNotificationChannel.ts` | Push delivery exposes no account-audience or subscription-filter registration API | The fork-owned adapter computes account-readable endpoints; the generic channel only accepts the resulting endpoint set and keeps payload/transport behavior upstream-owned | Session notification → account capability audience → bound Push endpoints → Web Push transport | Subscribe two accounts in one namespace, grant only one access, and verify delivery targets only the readable account endpoint |
...
| `cli/src/runtime/assets.ts` | Runtime readiness and staging are closed internals | Same-version runtime dirs must compare staged skill bytes with the embedded canonical payload and atomically restage mismatches | binary start → ensureRuntimeAssets → verified staged skill | Corrupt the staged skill while retaining the current version marker, restart, confirm canonical bytes are restored |
