...
| File | Necessary hook |
|---|---|
| `hub/src/startHub.ts` | Construct the gateway store and adapters; inject CLI, terminal, notification, account-memory, and web dependencies. |
| `hub/src/web/server.ts`, `hub/src/web/routes/messages.ts` | Mount gateway routes and authorization middleware; attach the authenticated gateway account id to web-originated messages. |
| `hub/src/sync/syncEngine.ts`, `hub/src/sync/messageService.ts` | Carry the account id to persistence, then decorate only copies delivered to the CLI while leaving stored and SSE message content unchanged. |
