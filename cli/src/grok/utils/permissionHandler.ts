...
export class GrokPermissionHandler extends BasePermissionHandler<PermissionResponseMessage, void> {
    ...
    private async autoApprove(
        ...
        logger.debug(`[Grok] Auto-approved ${toolName} (${request.id}) mode=${decision}`);
    }
    ...
    protected async handlePermissionResponse(
        ...
        if (pendingRequest) {
            ...
            logger.debug('[Grok] Permission response missing backend request', response.id);
        }
        ...
        logger.debug(`[Grok] Permission ${response.approved ? 'approved' : 'denied'} for ${pending.toolName}`);
