...
export class OmpPermissionHandler extends BasePermissionHandler<PermissionResponseMessage, void> {
    private readonly pendingBackendRequests = new Map<string, PermissionRequest>();

    constructor(
        session: ApiSessionClient,
        private readonly backend: AgentBackend,
        private readonly getPermissionMode: () => OmpPermissionMode | undefined
    ) {
        super(session);
    ...
    private handlePermissionRequest(request: PermissionRequest): void {
        ...
        logger.debug(`[Omp] Permission request queued for ${toolName} (${request.id})`);
    }
    ...
    private async autoApprove(
        ...
        logger.debug(`[Omp] Auto-approved ${toolName} (${request.id}) mode=${decision}`);
    }
    ...
    private async denyForPlanMode(
        ...
        logger.debug(`[Omp] Denied ${toolName} (${request.id}) in plan mode`);
    }
    ...
    protected async handlePermissionResponse(
        ...
        if (pendingRequest) {
            ...
            logger.debug('[Omp] Permission response missing backend request', response.id);
        }
        ...
        logger.debug(`[Omp] Permission ${response.approved ? 'approved' : 'denied'} for ${pending.toolName}`);
    ...
    protected handleMissingPendingResponse(response: PermissionResponseMessage): void {
        logger.debug('[Omp] Permission response received for unknown request', response.id);
    }
