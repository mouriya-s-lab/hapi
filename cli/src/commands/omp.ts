...
import { OMP_PERMISSION_MODES } from '@hapi/protocol/modes'
        ...
        try {
            ...
            for (let i = 0; i < commandArgs.length; i++) {
                ...
                if (arg === '--started-by') {
                    ...
                    if (!mode || !(OMP_PERMISSION_MODES as readonly string[]).includes(mode)) {
                        throw new Error(`Invalid --permission-mode value: ${mode ?? '(missing)'}`)
                    }
                    options.permissionMode = mode as OmpPermissionMode
